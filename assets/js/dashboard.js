...
function renderInvestSection() {
  const el = document.getElementById('dash-invest-kpi');
  if (!el) return;
  ...
  const profitBrut = valMarche !== null ? valMarche - valAchat : null;
  const profitPct  = profitBrut !== null && valAchat > 0 ? ((profitBrut/valAchat)*100).toFixed(1) : null;
  const sign       = profitBrut !== null && profitBrut >= 0 ? '+' : '';
  const col        = profitBrut !== null ? (profitBrut >= 0 ? 'var(--positive)' : 'var(--negative)') : '#ffb800';
